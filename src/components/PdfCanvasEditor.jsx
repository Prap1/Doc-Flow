...
export default function PdfCanvasEditor({ file, onCancel }) {
  ...
  useEffect(() => {
    ...
    const handleDelegatedClick = (e) => {
      ...
      const newEl = { 
        id: newId, type: 'text', x, y, text, page: pageNumber, 
        color: '#000000', size: fontSize, 
        bold: false, italic: false, underline: false, align: 'left',
        isReplacement: true, originalRect: { x, y, width: rect.width, height: rect.height }, originalText: text,
  ...
  useEffect(() => {
    const hideReplacedText = () => {
      ...
      const replacements = elements.filter(el => el.page === pageNumber && el.isReplacement);
      ...
      spans.forEach(span => {
        ...
        const x = rect.left - canvasRect.left;
        const y = rect.top - canvasRect.top;
        const isReplaced = replacements.some(rep => 
           rep.originalText.trim() === span.textContent.trim() &&
           Math.abs(rep.originalRect.x - x) < 30 && Math.abs(rep.originalRect.y - y) < 30
        );
        if (isReplaced) { 
  ...
  const handleImageUpload = (e) => {
    ...
    if (!file) { setActiveTool('selection'); return; }
  ...
  const handleCanvasMouseDown = (e) => {
    if (e.target.closest('.interactive-element') || e.target.closest('.mini-toolbar')) return;
    ...
    if (activeTool === 'draw') {
      ...
      const elToErase = elements.findLast(el => {
        ...
        if (el.type === 'highlight') {
          minX = Math.min(el.startX, el.endX); maxX = Math.max(el.startX, el.endX);
          minY = Math.min(el.startY, el.endY); maxY = Math.max(el.startY, el.endY);
        } else if (el.type === 'text') {
          minX = el.x; maxX = el.x + 100; minY = el.y; maxY = el.y + 30;
        } else if (el.type === 'image') {
          minX = el.x; maxX = el.x + el.width; minY = el.y; maxY = el.y + el.height;
        } else if (el.type === 'draw' || el.type === 'sign') {
          if (!el.path || el.path.length === 0) return false;
          minX = Math.min(...el.path.map(p => p.x)); maxX = Math.max(...el.path.map(p => p.x));
          minY = Math.min(...el.path.map(p => p.y)); maxY = Math.max(...el.path.map(p => p.y));
        } else if (el.type === 'cross' || el.type === 'check') {
          minX = el.x - 15; maxX = el.x + 15; minY = el.y - 15; maxY = el.y + 15;
        } else if (el.type === 'arrow') {
          minX = Math.min(el.startX, el.endX) - 10; maxX = Math.max(el.startX, el.endX) + 10;
          minY = Math.min(el.startY, el.endY) - 10; maxY = Math.max(el.startY, el.endY) + 10;
        }
        return x >= minX && x <= maxX && y >= minY && y <= maxY;
      });
      if (elToErase) saveHistory(elements.filter(el => el.id !== elToErase.id));
    }
  };
  ...
  const handleCanvasClick = (e) => {
    ...
    if (activeTool === 'text') {
      ...
      const newEl = { 
        id: newId, type: 'text', x, y, text: '', page: pageNumber, 
        color: selectedColor, size: textSize, bold: isBold, italic: isItalic, underline: isUnderline, align: textAlign,
      ...
      saveHistory([...elements, {
        id: newId, type: 'image', x, y, width: 200, height: 200, src: pendingImage, page: pageNumber,
        canvasWidth: rect.width, canvasHeight: rect.height
      }]);
      ...
      const newId = Date.now();
      saveHistory([...elements, { 
        id: newId, type: activeTool, x, y, page: pageNumber, color: selectedColor,
        canvasWidth: rect.width, canvasHeight: rect.height
      }]);
    }
  };
      ...
      for (const el of currentElements) {
        ...
        if (el.type === 'text') {
          ...
          if (el.isReplacement && el.originalRect) {
            const rx = el.originalRect.x * scaleX, ry = pdfH - (el.originalRect.y + el.originalRect.height) * scaleY;
            const rw = el.originalRect.width * scaleX, rh = el.originalRect.height * scaleY;
            page.drawRectangle({ x: rx - 1, y: ry - 2, width: rw + 2, height: rh + 4, color: rgb(1, 1, 1) });
          }
          ...
          if (el.text) {
            page.drawText(el.text, { x: tx, y: ty, size: fontSize, font: font, color: rgb(r, g, b) });
            if (el.underline) {
              const textWidth = font.widthOfTextAtSize(el.text, fontSize);
              page.drawLine({ start: { x: tx, y: ty - 2 }, end: { x: tx + textWidth, y: ty - 2 }, thickness: Math.max(1, fontSize * 0.05), color: rgb(r, g, b) });
            }
          }
        } else if (el.type === 'image') {
          const imageBytes = await fetch(el.src).then(res => res.arrayBuffer());
          let pdfImage;
          if (el.src.startsWith("data:image/png")) { pdfImage = await pdfDoc.embedPng(imageBytes); } 
          else { pdfImage = await pdfDoc.embedJpg(imageBytes); }
          page.drawImage(pdfImage, { x: el.x * scaleX, y: pdfH - (el.y + el.height) * scaleY, width: el.width * scaleX, height: el.height * scaleY });
        } else if (el.type === 'draw' || el.type === 'sign') {
          if (el.path.length > 1) {
            const sx = pdfW / (el.canvasWidth || pdfW), sy = pdfH / (el.canvasHeight || pdfH);
            for (let i = 0; i < el.path.length - 1; i++) {
              page.drawLine({
                start: { x: el.path[i].x * sx, y: pdfH - el.path[i].y * sy }, end: { x: el.path[i+1].x * sx, y: pdfH - el.path[i+1].y * sy },
                thickness: 2, color: rgb(r, g, b),
              });
            }
          }
        } else if (el.type === 'arrow') {
          const sx = pdfW / (el.canvasWidth || pdfW), sy = pdfH / (el.canvasHeight || pdfH);
          const start = { x: el.startX * sx, y: pdfH - el.startY * sy }, end = { x: el.endX * sx, y: pdfH - el.endY * sy };
          page.drawLine({ start, end, thickness: 2, color: rgb(r, g, b) });
          const angle = Math.atan2(end.y - start.y, end.x - start.x);
          const headlen = 10;
          page.drawLine({ start: end, end: { x: end.x - headlen * Math.cos(angle - Math.PI / 6), y: end.y - headlen * Math.sin(angle - Math.PI / 6) }, thickness: 2, color: rgb(r, g, b) });
          page.drawLine({ start: end, end: { x: end.x - headlen * Math.cos(angle + Math.PI / 6), y: end.y - headlen * Math.sin(angle + Math.PI / 6) }, thickness: 2, color: rgb(r, g, b) });
        } else if (el.type === 'check' || el.type === 'cross') {
          const sx = pdfW / (el.canvasWidth || pdfW), sy = pdfH / (el.canvasHeight || pdfH);
          const cx = el.x * sx, cy = pdfH - el.y * sy, size = 15;
          if (el.type === 'check') {
            page.drawLine({ start: { x: cx - size/2, y: cy }, end: { x: cx - size/6, y: cy - size/2 }, thickness: 2, color: rgb(r, g, b) });
            page.drawLine({ start: { x: cx - size/6, y: cy - size/2 }, end: { x: cx + size/2, y: cy + size/2 }, thickness: 2, color: rgb(r, g, b) });
          } else {
            page.drawLine({ start: { x: cx - size/2, y: cy - size/2 }, end: { x: cx + size/2, y: cy + size/2 }, thickness: 2, color: rgb(r, g, b) });
            page.drawLine({ start: { x: cx - size/2, y: cy + size/2 }, end: { x: cx + size/2, y: cy - size/2 }, thickness: 2, color: rgb(r, g, b) });
          }
        } else if (el.type === 'highlight') {
          const sx = pdfW / (el.canvasWidth || pdfW), sy = pdfH / (el.canvasHeight || pdfH);
          const rectX = Math.min(el.startX, el.endX) * sx, rectY = Math.min(el.startY, el.endY) * sy;
          const rectW = Math.abs(el.endX - el.startX) * sx, rectH = Math.abs(el.endY - el.startY) * sy;
          page.drawRectangle({ x: rectX, y: pdfH - rectY - rectH, width: rectW, height: rectH, color: rgb(r, g, b), opacity: el.opacity });
        }
      }
  ...
  const updateElementAndSaveHistory = (id, newProps) => saveHistory(elements.map(el => el.id === id ? { ...el, ...newProps } : el));
  const deleteElement = (id) => { saveHistory(elements.filter(el => el.id !== id)); setSelectedElementId(null); };
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden bg-gray-100/50">
             ...
             <div className="relative border border-gray-200 bg-white ring-1 ring-gray-900/5">
               ...
               >
                  {elements.filter(el => el.page === pageNumber).map(el => {
                     ...
                     if (el.type === 'image') {
                       return (
                         <motion.div key={el.id} className="interactive-element absolute"
                           style={{ left: 0, top: 0, x: el.x, y: el.y, width: el.width, height: el.height, pointerEvents: 'auto' }}
                           drag={activeTool === 'selection'} dragMomentum={false}
                     ...
                     if (el.type === 'text') {
                        if (el.isReplacement) {
                           return (
                              <React.Fragment key={el.id}>
                                 <div className="absolute bg-white" style={{ left: el.originalRect.x, top: el.originalRect.y, width: el.originalRect.width + 8, height: el.originalRect.height + 4, zIndex: 49, pointerEvents: 'none' }} />
                                 <div className="absolute" style={{ left: el.x, top: el.y, zIndex: 60, pointerEvents: 'auto' }} onClick={(e) => { e.stopPropagation(); setSelectedElementId(el.id); }}>
                                    <input type="text" value={el.text} onChange={(e) => updateElement(el.id, { text: e.target.value })}
                                      onBlur={() => updateElementAndSaveHistory(el.id, { text: el.text })}
                                    ...
                                    {isSelected && <button className="absolute -top-5 right-0 text-[10px] bg-red-100 hover:bg-red-200 text-red-600 rounded px-1" onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); deleteElement(el.id); }}>✕</button>}
                                 </div>
                              </React.Fragment>
                           );
                        }
                        ...
                        return (
                          <motion.div key={el.id} className="interactive-element absolute"
                            style={{ left: 0, top: 0, x: el.x, y: el.y, pointerEvents: 'auto' }}
                            drag={activeTool === 'selection'} dragMomentum={false}
                            onDragEnd={(e, info) => updateElementAndSaveHistory(el.id, { x: el.x + info.offset.x, y: el.y + info.offset.y })}
                            onClick={(e) => { e.stopPropagation(); setActiveTool('selection'); setSelectedElementId(el.id); }}
                          >
                     ...
                     if (el.type === 'highlight') {
                        return <div key={el.id} style={{ position: 'absolute', left: Math.min(el.startX, el.endX), top: Math.min(el.startY, el.endY), width: Math.abs(el.endX - el.startX), height: Math.abs(el.endY - el.startY), backgroundColor: el.color, opacity: el.opacity, pointerEvents: 'none' }} />;
                     }
                     return null;
                  })}
                  
                  <svg className="absolute top-0 left-0 w-full h-full pointer-events-none">
                     {elements.filter(el => el.page === pageNumber && (el.type === 'draw' || el.type === 'sign')).map(el => (
                        <polyline key={el.id} points={el.path.map(p => `${p.x},${p.y}`).join(' ')} fill="none" stroke={el.color || 'red'} strokeWidth="2" />
                     ))}
                     {elements.filter(el => el.page === pageNumber && el.type === 'arrow').map(el => {
                        const angle = Math.atan2(el.endY - el.startY, el.endX - el.startX);
                        const headlen = 10;
                        return (
                          <g key={el.id} stroke={el.color || 'red'} strokeWidth="2" fill="none">
                            <line x1={el.startX} y1={el.startY} x2={el.endX} y2={el.endY} />
                            <line x1={el.endX} y1={el.endY} x2={el.endX - headlen * Math.cos(angle - Math.PI / 6)} y2={el.endY - headlen * Math.sin(angle - Math.PI / 6)} />
                            <line x1={el.endX} y1={el.endY} x2={el.endX - headlen * Math.cos(angle + Math.PI / 6)} y2={el.endY - headlen * Math.sin(angle + Math.PI / 6)} />
                          </g>
                        );
                     })}
                     {elements.filter(el => el.page === pageNumber && el.type === 'check').map(el => (
                        <g key={el.id} stroke={el.color || 'black'} strokeWidth="2" fill="none">
                          <line x1={el.x - 7.5} y1={el.y} x2={el.x - 2.5} y2={el.y - 7.5} />
                          <line x1={el.x - 2.5} y1={el.y - 7.5} x2={el.x + 7.5} y2={el.y + 7.5} />
                        </g>
                     ))}
                     {elements.filter(el => el.page === pageNumber && el.type === 'cross').map(el => (
                        <g key={el.id} stroke={el.color || 'black'} strokeWidth="2" fill="none">
        ...
        {numPages > 1 && (
           <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex items-center bg-white shadow-xl rounded-full px-4 py-2 border border-gray-100 z-20">
             <button className="p-1 hover:bg-gray-100 rounded disabled:opacity-30" onClick={() => setPageNumber(p => Math.max(1, p - 1))} disabled={pageNumber <= 1}>&lt;</button>
             <span className="text-sm font-medium mx-4">Page {pageNumber} of {numPages}</span>
             <button className="p-1 hover:bg-gray-100 rounded disabled:opacity-30" onClick={() => setPageNumber(p => Math.min(numPages, p + 1))} disabled={pageNumber >= numPages}>&gt;</button>
           </div>
        )}
      </div>
         ...
         <div className="w-64 bg-white border-l border-gray-200 flex flex-col p-5 gap-6 z-20 shadow-sm shrink-0 overflow-y-auto">
            <div>
               ...
               <div className="grid grid-cols-5 gap-2">
                 {COLORS.map(c => (
                   <button key={c} onClick={() => { setSelectedColor(c); if(selectedElementId) updateElementAndSaveHistory(selectedElementId, { color: c }); }} className="w-8 h-8 rounded-full border border-gray-200 shadow-sm transition-transform hover:scale-110" style={{ backgroundColor: c, ringColor: selectedColor === c ? '#3b82f6' : 'transparent', ringWidth: selectedColor === c ? '2px' : '0' }} />
                 ))}
               </div>
            </div>
               ...
               <>
                 ...
                 <div>
                   ...
                   <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-1 mb-2">
                     <button onClick={() => toggleFormat('bold', isBold, setIsBold)} className={`flex-1 p-2 rounded ${isBold ? 'bg-white shadow-sm font-bold text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}>B</button>
                     <button onClick={() => toggleFormat('italic', isItalic, setIsItalic)} className={`flex-1 p-2 rounded italic ${isItalic ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}>I</button>
                     <button onClick={() => toggleFormat('underline', isUnderline, setIsUnderline)} className={`flex-1 p-2 rounded underline ${isUnderline ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}>U</button>
                   </div>
                   <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-1">
                     <button onClick={() => applyAlign('left')} className={`flex-1 p-2 flex justify-center rounded ${textAlign === 'left' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}><AlignLeft size={16} /></button>
                     <button onClick={() => applyAlign('center')} className={`flex-1 p-2 flex justify-center rounded ${textAlign === 'center' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}><AlignCenter size={16} /></button>
                     <button onClick={() => applyAlign('right')} className={`flex-1 p-2 flex justify-center rounded ${textAlign === 'right' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}><AlignRight size={16} /></button>
                   </div>
                 </div>
            ...
            {selectedElementId !== null && (
               <>
                 <div className="h-[1px] w-full bg-gray-100" />
                 <button className="w-full py-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg font-medium text-sm transition flex items-center justify-center gap-2" onClick={() => deleteElement(selectedElementId)}>
