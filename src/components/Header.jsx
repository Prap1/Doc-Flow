...
import { Search, Sparkles, Home, FileText, File, BookOpen, FileSpreadsheet, Image as ImageIcon, MessageSquare, MessagesSquare, ChevronDown, ChevronRight, Wrench, Share2 } from 'lucide-react';
import Logo from '../assets/Logo';

const toolsDropdown = [
  { path: '/word', icon: <File size={16} />, label: 'Word Studio', color: 'text-cyan-400' },
  { path: '/docs', icon: <BookOpen size={16} />, label: 'Docs Studio', color: 'text-emerald-400' },
  { path: '/excel', icon: <FileSpreadsheet size={16} />, label: 'Excel Studio', color: 'text-orange-400' },
  { path: '/image', icon: <ImageIcon size={16} />, label: 'Image Studio', color: 'text-purple-400' },
];

const socialDropdown = [
  { path: '/chat', icon: <MessageSquare size={16} />, label: 'Post Studio', color: 'text-rose-400' },
  { path: '/whatsapp', icon: <MessagesSquare size={16} />, label: 'Chat Studio', color: 'text-green-400' },
];

const DropdownItem = ({ item }) => (
  <NavLink
    to={item.path}
    className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 hover:bg-white/10 transition-colors ${isActive ? 'bg-white/5' : ''}`}
  >
...
const NavItem = ({ path, icon, label, color, activeBg, border, badge }) => (
  <NavLink to={path} end={path === '/'} className="flex-shrink-0">
    {({ isActive }) => (
      <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all duration-200 cursor-pointer ${isActive ? `${activeBg} ${border} ${color} shadow-[0_0_16px_rgba(var(--tw-shadow-color),0.2)]` : 'border-transparent text-white/50 hover:bg-white/5 hover:text-white/80'}`}>
        <div className={isActive ? color : ''}>{icon}</div>
        <span className={`text-sm whitespace-nowrap ${isActive ? 'font-semibold' : 'font-medium'}`}>{label}</span>
        {badge && (
      ...
      {isOpen && (
        <div className="absolute top-full left-0 pt-2 z-50">
          <div className="bg-[#12122A] border border-white/10 rounded-xl shadow-xl overflow-hidden min-w-[160px] py-1 animate-in fade-in slide-in-from-top-2 duration-200">
            {items.map(item => <DropdownItem key={item.path} item={item} />)}
          </div>
        </div>
      )}
    </div>
      ...
      <div className="flex items-center gap-6 flex-1 h-full">
        <div className="flex items-center gap-2">
          ...
          <div className="font-outfit font-extrabold text-lg tracking-tight whitespace-nowrap">
            <span className="bg-clip-text text-transparent bg-gradient-to-br from-indigo-400 to-cyan-400">DocFlow</span>
        ...
        <nav className="flex items-center gap-2 h-full pt-1">
          <NavItem path="/" icon={<Home size={16} />} label="Home" color="text-indigo-400" activeBg="bg-indigo-500/20" border="border-indigo-500/40" />
          <DropdownNav icon={<Wrench size={16} />} label="Tools" items={toolsDropdown} />
          <NavItem path="/pdf" icon={<FileText size={16} />} label="PDF Studio" color="text-pink-400" activeBg="bg-pink-500/20" border="border-pink-500/40" badge="PDF" />
          <DropdownNav icon={<Share2 size={16} />} label="Social Studio" items={socialDropdown} />
        </nav>
      </div>
