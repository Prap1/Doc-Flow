...
// ── PDF ──────────────────────────────────────────────────────
export async function pdfAnnotate(
  file,
  annotationText,
  page = 0,
  x = 50,
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/pdf/annotate`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function pdfMerge(files) {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/pdf/merge`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function pdfSplit(file, pages = "") {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/pdf/split`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function pdfInfo(file) {
  ...
  fd.append("file", file);
  const res = await handleResponse(
    await fetch(`${BASE}/pdf/info`, { method: "POST", body: fd }),
  );
  return res.json();
}
...
export async function wordMerge(files) {
  ...
  files.forEach((f) => fd.append("files", f));
  const res = await handleResponse(
    await fetch(`${BASE}/word/merge`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function wordSplit(file) {
  ...
  fd.append("file", file);
  const res = await handleResponse(
    await fetch(`${BASE}/word/split`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function wordToHtml(file) {
  ...
  fd.append("file", file);
  const res = await handleResponse(
    await fetch(`${BASE}/word/convert/html`, { method: "POST", body: fd }),
  );
...
export async function htmlToDocx(htmlContent, title = "Document") {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/word/convert/docx`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function excelMerge(files) {
  ...
  files.forEach((f) => fd.append("files", f));
  const res = await handleResponse(
    await fetch(`${BASE}/excel/merge`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function excelSplit(file) {
  ...
  fd.append("file", file);
  const res = await handleResponse(
    await fetch(`${BASE}/excel/split`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function excelToJson(file) {
  ...
  fd.append("file", file);
  const res = await handleResponse(
    await fetch(`${BASE}/excel/to-json`, { method: "POST", body: fd }),
  );
...
export async function jsonToExcel(data, sheetName = "Sheet1") {
  const res = await handleResponse(
    await fetch(`${BASE}/excel/from-json`, {
      method: "POST",
...
export async function imageEdit(file, params = {}) {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/image/edit`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function imageMerge(files, direction = "horizontal") {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/image/merge`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function imageSplit(file, parts = 2, direction = "horizontal") {
  ...
  fd.append("direction", direction);
  const res = await handleResponse(
    await fetch(`${BASE}/image/split`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function imageCompress(file, quality = 75, maxWidth = 1920) {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/image/compress`, { method: "POST", body: fd }),
  );
  return res.blob();
}
...
export async function reactToPost(id, key) {
  const res = await handleResponse(
    await fetch(`${BASE}/posts/${id}/react`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function uploadVoice(postId, audioBlob) {
  ...
  const res = await handleResponse(
    await fetch(`${BASE}/posts/${postId}/voice`, {
      method: "POST",
      body: fd,
    }),
  );
