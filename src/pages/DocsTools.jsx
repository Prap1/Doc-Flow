...
export default function DocsTools() {
  ...
  const [docs, setDocs] = useState([
    { id: 1, title: "Untitled Doc", content: "" },
  ]);
  ...
  const exportAs = (format) => {
    ...
    if (format === "txt") {
      ...
      saveAs(
        new Blob([tmp.textContent], { type: "text/plain" }),
        `${d.title}.txt`,
      );
    } else {
      saveAs(
        new Blob([d.content], { type: "text/html" }),
        `${d.title}.${format}`,
      );
    }
  ...
  const handleMerge = () => {
    if (docs.length < 2)
      return showToast("Add at least 2 documents to merge", "error");
    const combined = docs
      .map((d, i) => `<h2>Document ${i + 1}: ${d.title}</h2>${d.content}<hr/>`)
      .join("");
  ...
  const handleSplit = () => {
    ...
    const p2 = [...paras]
      ...
      .join("");
    saveAs(
      new Blob([`<html><body>${p1}</body></html>`], { type: "text/html" }),
      `${d.title}_part1.html`,
    );
    saveAs(
      new Blob([`<html><body>${p2}</body></html>`], { type: "text/html" }),
      `${d.title}_part2.html`,
    );
            ...
            <div>
              ...
              <p style={{ color: "rgba(240,240,255,0.5)", fontSize: 15 }}>
                {activeTool !== null
                  ...
                  : "Create, Edit, Merge, Split & Convert documents"}
              </p>
            </div>
          </div>
          <div
              ...
              <div className="tool-content">
                  ...
                  {docs.map((d, i) => (
                    <button
                      ...
                      style={{
                        padding: "6px 16px",
                        borderRadius: 8,
                        border: `1px solid ${i === activeDoc ? toolColor + "50" : "rgba(255,255,255,0.08)"}`,
                        background:
                          i === activeDoc
                            ? `${toolColor}15`
                            : "rgba(255,255,255,0.04)",
                        color:
                          i === activeDoc ? toolColor : "rgba(240,240,255,0.5)",
                        fontSize: 13,
                        cursor: "pointer",
                        whiteSpace: "nowrap",
                      }}
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDocs((p) =>
                      p.map((d, i) =>
                        i === activeDoc ? { ...d, title: e.target.value } : d,
                      ),
                    )
                  }
            ...
            {activeTool === 1 && (
              <div className="tool-content">
                <div className="card" style={{ padding: 24, marginBottom: 16 }}>
                  ...
                  >
                    You have{" "}
                    <strong style={{ color: toolColor }}>
                      {docs.length} document(s)
                    </strong>{" "}
                    in your workspace. Click "Merge All" to combine them into
                    one file.
                  </p>
                  {docs.map((d, i) => (
                    <div
                    ...
                    >
                      ...
                      <div className="file-item-info">
                        <div className="file-item-name">{d.title}</div>
                      </div>
                    </div>
                  ))}
                </div>
                <button className="btn btn-primary" onClick={handleMerge}>
                  <Merge size={15} /> Merge All Documents
                </button>
              </div>
            )}
            ...
            {activeTool === 2 && (
              <div className="tool-content">
                <div className="card" style={{ padding: 24, marginBottom: 16 }}>
                  ...
                  >
                    Split active document:{" "}
                    <strong style={{ color: toolColor }}>
                      {docs[activeDoc]?.title}
                    </strong>
                  </p>
                  <p style={{ fontSize: 13, color: "rgba(240,240,255,0.4)" }}>
                    Document will be split into 2 equal parts by paragraphs.
                  </p>
                </div>
            ...
            {activeTool === 3 && (
              <div className="tool-content">
                ...
                >
                  {[
                    {
                      label: "Export → Word (.docx concept)",
                      emoji: "📝",
                      color: "#00D9FF",
                      format: "html",
                    },
                    {
                      label: "Export → PDF concept",
                      emoji: "📄",
                      color: "#FF6B9D",
                      format: "html",
                    },
                    ...
                    >
                      ...
                      <p style={{ fontSize: 13, color: "#555" }}>
                        Click to convert and download current document
                      </p>
                    </motion.div>
                  ))}
                </div>
