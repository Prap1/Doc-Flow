...
export default function ImageTools() {
  ...
  const handleMergeImages = () => {
    if (mergeImages.length < 2)
      return showToast("Add at least 2 images", "error");
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
  ...
  const handleSplitImage = () => {
    ...
    im.onload = () => {
      ...
      [
        ...
        c.toBlob(
          (blob) => saveAs(blob, `part${i + 1}_${image?.name || "image.png"}`),
          "image/png",
        );
      });
        ...
        <div className="page-header">
          ...
          >
            <span style={{ fontSize: 40 }}>
              {activeTool !== null ? TOOLS[activeTool].icon : "🖼️"}
            </span>
            <div>
              <h1
                style={{
              ...
              <div className="tool-content">
                <FileDropzone
                  onFiles={(f) =>
                    setMergeImages((p) => [
                      ...p,
                      ...f.map((fi) => URL.createObjectURL(fi)),
                    ])
                  }
                  accept={{ "image/*": [".jpg", ".jpeg", ".png", ".webp"] }}
                  label="Add images to merge (stitch side by side)"
                  icon="🖼️"
                  color={toolColor}
                />
                {mergeImages.length > 0 && (
                  ...
                  >
                    ...
                    >
                      {mergeImages.map((url, i) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              setMergeImages((p) => p.filter((_, j) => j !== i))
                            }
                            style={{
                              position: "absolute",
                              top: -6,
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      >
                        <Merge size={15} /> Merge & Preview {mergeImages.length}{" "}
                        Images
                      </button>
                      <button
                        className="btn btn-secondary"
                        onClick={() => setMergeImages([])}
                      >
              ...
              <div className="tool-content">
                {!imgUrl ? (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    >
                      Image will be split vertically into 2 equal halves.
                    </p>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button
                        className="btn btn-accent"
