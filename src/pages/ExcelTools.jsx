...
export default function ExcelTools() {
  ...
  const addRow = () =>
    setTableData((p) => [...p, Array(p[0]?.length || 5).fill("")]);
  ...
  const handleMerge = async () => {
    if (mergeFiles.length < 2)
      return showToast("Add at least 2 files", "error");
    try {
      ...
      if (e.data instanceof Blob) {
        try {
          msg = JSON.parse(await e.data.text()).error || msg;
        } catch {}
            ...
            <div>
              ...
              <p style={{ color: "rgba(240,240,255,0.5)", fontSize: 15 }}>
                {activeTool !== null
                  ...
                  : "Edit, Merge & Split Excel spreadsheets"}
              </p>
            </div>
          </div>
          <div
              ...
              <div className="tool-content">
                {!workbook ? (
                  <FileDropzone
                    ...
                    multiple={false}
                    label="Drop your Excel file (.xlsx / .xls)"
                    icon="📊"
                    color={toolColor}
                  />
                ) : (
                        ...
                        {sheetNames.map((name, i) => (
                          <button
                            ...
                            style={{
                              padding: "5px 14px",
                              borderRadius: 8,
                              border: `1px solid ${i === activeSheet ? toolColor + "50" : "rgba(255,255,255,0.1)"}`,
                              background:
                                i === activeSheet
                                  ? `${toolColor}15`
            ...
            {activeTool === 1 && (
              <div className="tool-content">
                <FileDropzone
                  ...
                  label="Add Excel files to merge"
                  icon="🔗"
                  color={toolColor}
                />
                {mergeFiles.length > 0 && (
                  ...
                  >
                    ...
                    <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
                      ...
                      >
                        ...
                        {processing
                          ? "Merging…"
                          : `Merge & Preview ${mergeFiles.length} Sheets`}
                      </button>
                      <button
                        className="btn btn-secondary"
            ...
            {activeTool === 2 && (
              <div className="tool-content">
                <FileDropzone
                  ...
                  multiple={false}
                  label="Drop Excel file to split by sheets"
                  icon="✂️"
                  color={toolColor}
                />
                {splitFile && (
                  ...
                  >
                    ...
                    >
                      Each sheet will be saved as a separate .xlsx file.
                    </p>
                    <button
                      className="btn btn-primary"
                      onClick={() => handleSplit(splitFile)}
                      disabled={processing}
                    >
                      {processing ? (
                        <RefreshCw size={15} />
                      ...
                      {processing ? "Splitting…" : "Split & Preview Sheets"}
                    </button>
                  </motion.div>
                )}
              </div>
